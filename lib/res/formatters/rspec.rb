    ...
    class Rspec
      ...
      def example_group_started(group_notification)
        if @child == 0
          ...
          @child += 1
          i = 0
          index = if result[@count].key?(:children)
                    result[@count][:children].count
                  else
                    0
                  end
          while i < group_notification.group.examples.count
            result[@count][:children] = [] unless result[@count].key?(:children)
        ...
        unless group.children.empty?
          ...
          while i < group.children.count
            ...
            while k < group.children[i].examples.count
              if k == 0
                result[:children][j][:children] = []
                result[:children][j][:children][k] = {}
              end
      ...
      def start_dump(_start_dump_notification)
