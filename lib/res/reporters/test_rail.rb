...
module Res
  module Reporters
    class TestRail
      ...
      def initialize(args)
        @url = args[:url]
        @config = Res::Config.new(%i[user password namespace project suite], optional: %i[plan_name run_id run_name], pre_env: 'testrail_')
        config.process(args)
      ...
      def add_case_status(run_id)
        @case_status.each do |case_id, status|
