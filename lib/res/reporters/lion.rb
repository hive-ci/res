  ...
  module Reporters
    class Lion
      ...
      def initialize(args)
        ...
        # required list.
        @config = Res::Config.new(%i[url tag description app_name target],
                                  optional: %i[hive_job_id queue cert cacert ssl_verify_mode device_type version],
                                  pre_env: 'LION_')
        config.process(args)
      end
      ...
      def submit_results(ir, _args = nil)
        ...
        if ir.values.key?('#type') && ir.values['#type'] == 'multiple'
          measures = {}
          ir.values.reject { |k, _v| k =~ /^[#_]/ }.each do |k, v|
