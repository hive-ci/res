...
module Res
  class IR
    ...
    def initialize(options = {})
      (@results     = options[:results])     || raise('No results data')
      (@type        = options[:type])        || raise("No type provided (e.g. 'Cucumber')")
      (@started     = options[:started])     || raise('Need to provide a start time')
      (@finished    = options[:finished])    || raise('Need to provide an end time')
    ...
    def self.find_values(nodes)
      value_hashes = []
      nodes.each do |n|
        value_hashes << n[:values] if n[:values]
        value_hashes += IR.find_values(n[:children]) if n[:children]
      end
      value_hashes
    end
